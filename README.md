...
- 涨跌停按股票属性识别：主板 / ST 10%、创业板 / 科创板 20%、北交所 30%；上市未满两周、规则无法可靠判断的股票不计入涨跌停数量。
...
数据源为东方财富公开行情接口。
- 实时行情与主力资金：东方财富行情接口
- 涨停池与涨停梯队历史：东方财富涨停专题池接口
- 接口异常时：优先展示最近一次有效数据，并标注延迟或失败状态
...
- 行情全量刷新为 10 秒；
- 当前选中的池（正在查看个股明细的“个股池”） 5 秒刷新；
- ”梯队回看“部分为 30 秒；
- 只在数值变化时更新前端渲染；
- 由于数据源为免费的http api，短时间内请求次数有效，则项目按单次 80 只分批请求，一批次完成数据刷新后，才会进入下一批次的数据请求；
